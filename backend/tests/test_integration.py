...
class _TestVectorStore:
    ...
    def __init__(self) -> None:
        ...
        # chromadb's in-memory system is process-global, so a fixed collection
        # name leaks rows across tests (part2's counts saw part1's leftovers).
        # Unique per instance = full isolation.
        self._collection_name = f"veridoc_documents_{uuid.uuid4().hex[:8]}"
        try:
                ...
                chunks.append(
                    {
                        ...
                        "score": (
                            1.0 - results["distances"][0][i] if results.get("distances") else 0.0
                        ),
                        "source": "vector",
                    }
                )
...
async def test_process_document_end_to_end(
    ...
    doc_id = uuid.uuid4()
    user = User(id=uuid.uuid4(), email=f"ingest-{uuid.uuid4().hex[:8]}@example.com", role="user")
    doc = Document(
        id=doc_id,
        user_id=user.id,
...
async def test_postgres_document_chunk_relationship(pg_session, temp_dir) -> None:
    ...
    user = User(id=uuid.uuid4(), email=f"rel-{uuid.uuid4().hex[:8]}@example.com", role="user")
    doc = Document(
        id=uuid.uuid4(),
        user_id=user.id,
        title="Test Document",
